...
- `src/Result/` — the library itself (`Result.csproj`, targets `netstandard2.0`). This is what gets packed and published to NuGet.
  ...
  - `Extensions/` — `ResultExtensions` (`IsFailed`, `ToHttpStatusCode`, `ToPaged`, `ToPagedResult`)
...
2. **Implicit operators never throw.** `null` input to an implicit conversion must produce an Error result or `null`/`default` output — never a custom exception. Only explicit methods with required parameters throw (`ArgumentNullException` for `new ResultCode(null)`, `Result.From(null)`, `ToPaged(null list)`, etc.). If you add an implicit operator, follow the existing behavior matrix in `README.md`.
...
5. Public API changes to `src/Result` are NuGet package surface (current version `2.0.0.0` in the csproj) — treat removals/signature changes as breaking and call it out explicitly.
